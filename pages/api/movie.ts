...
export async function getTodayTrend() {
  const movieListReq = await axios.get(
    `https://api.themoviedb.org/3/trending/movie/day?api_key=${process.env.NEXT_PUBLIC_TMDB_KEY}&page=1&language=ko-KR`
  );

  const movieList = await movieListReq.data.results.slice(0, 2);
